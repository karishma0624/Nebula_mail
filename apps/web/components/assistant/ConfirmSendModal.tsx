...
export const ConfirmSendModal: React.FC = () => {
  ...
  const handleConfirmSend = async () => {
    // Read the most recent draftToSend live from state at click time to prevent stale closures (Section 13)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="max-w-lg w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4">
        ...
        <div className="space-y-2.5 bg-slate-50 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 text-xs">
          <div>
            <span className="text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Recipient:</span>
            <p className="text-slate-900 dark:text-slate-100 font-medium mt-0.5">{draftToSend.to}</p>
          </div>
          <div>
            <span className="text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Subject:</span>
            <p className="text-slate-900 dark:text-slate-100 font-medium mt-0.5">{draftToSend.subject}</p>
          </div>
          <div>
            <span className="text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-[10px]">Body Preview:</span>
            <div className="text-slate-800 dark:text-slate-200 mt-1 max-h-36 overflow-y-auto whitespace-pre-wrap font-mono text-[11px] bg-white dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800/80">
              {draftToSend.body}
            </div>
          </div>
        </div>
